...
IMPORTANT: Analyze ALL data thoroughly. Reference specific test names, dates, numbers.

Respond ONLY in valid JSON with these sections:
- executive_summary (4-6 sentences with specific numbers)
- performance (current_month with revenue/spend targets vs actuals, MER, year_to_date totals, monthly_trend array, yoy_comparison, performance_commentary)
- creative_roadmap_breakdown (total counts, hit_rate, tests array listing EVERY test with name/batch/author/type/result/learnings, winning_patterns, losing_patterns, biggest_winner, gaps_in_testing, velocity)
- top_performing_ads (name, delivery, spend, assessment per ad)
- fatigue_alerts (ad_name, signal, urgency, action)
- weekly_creative_briefs (5-8 briefs each with priority, concept, format, angle, awareness_level, hook_direction, body_direction, cta_direction, why_this_week, reference, expected_impact)
- cro_insights (tests array, key_finding, next_recommendation)
- awareness_analysis (levels_tested counts, gaps, recommendation)
- desire_alignment (desires identified, addressed, not_addressed, recommendation)
- media_buying_notes (recent_changes, channel_status, recommendation)
- upcoming_events (from Key Events tab)
- pending_actions (from meetings)
- priorities_this_week (5 items each with priority, action, owner: Media Buying/Creative/Ops/Client, why)
- risk_flags (flag, severity: high/medium/low, mitigation)
- client_health (score: green/yellow/red, reason, 30_day_outlook)

Be brutally specific. Reference actual test names. Connect performance to creative. List every test.`;
...
async function fetchSheet(sheetUrl, googleApiKey) {
  ...
  const TAB_MAP = {
    ...
    roadmap: tabNames.find(t => t.toLowerCase().includes('creative roadmap')),
    ...
    keyEvents: tabNames.find(t => t.toLowerCase().includes('key events')),
    ...
    for (const client of clients) {
      try {
        ...
        const aRes = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            model: 'claude-sonnet-4-20250514', max_tokens: 16000, system: AGENT_PROMPT,
